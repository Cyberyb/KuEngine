...
REM One-click build + shader compile + run for CubeApp
...
echo [2/4] Build CubeApp (%CONFIG%)...
cmake --build "%BUILD_DIR%" --config "%CONFIG%" --target CubeApp
if errorlevel 1 (
    echo Build failed.
    exit /b 1
)
...
echo [4/4] Run CubeApp...
pushd "%RUN_DIR%"
start "CubeApp" "CubeApp.exe"
popd
