...
REM One-click build + shader compile + run for Alpha3PassApp
...
echo [2/4] Build Alpha3PassApp (%CONFIG%)...
cmake --build "%BUILD_DIR%" --config "%CONFIG%" --target Alpha3PassApp
if errorlevel 1 (
    echo Build failed.
    exit /b 1
)
...
echo [4/4] Run Alpha3PassApp...
pushd "%RUN_DIR%"
start "Alpha3PassApp" "Alpha3PassApp.exe"
popd
